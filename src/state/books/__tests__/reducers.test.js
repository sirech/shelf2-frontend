...
describe('book reducer', () => {
  ...
  describe('handles BOOK_CREATE_SUCCESS', () => {
    it('does not do anything if it is not the same year', () => {
      ...
      expect(reducer(books(), { type: constants.BOOK_CREATE_SUCCESS, payload: book })).toEqual(books())
    })

    it('adds the book if it is in the same year', () => {
      const book = factories.book.build({ title: 'My book', category: 'software', stars: 3, year: 2016 })
      expect(reducer(books(), { type: constants.BOOK_CREATE_SUCCESS, payload: book })).toMatchSnapshot()
    })
  })
})
